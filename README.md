...
- [Смена номера / SIP-аккаунта](#-смена-номера--sip-аккаунта)
...
## 📞 Смена номера / SIP-аккаунта

Два разных случая, которые легко перепутать:

**Caller ID (номер, который видит абонент на экране телефона) и AMI-доступ** — настраиваются прямо в веб-интерфейсе, «Настройки → Обзвон» и «Настройки → Asterisk» соответственно. `.env` трогать не нужно. Изменение применяется после перезапуска бэкенда (кнопка «Перезагрузить сервисы» в интерфейсе или `autodialer-restart`).

**SIP-аккаунт (extension), через который реально идут звонки на FreePBX** — это не настройка бэкенда, а регистрация SIP-транка на стороне Asterisk, поэтому в веб-интерфейсе её нет и не должно быть: это конфиг телефонии, генерируемый один раз при установке из `scripts/04_pjsip_config.sh` в `/etc/asterisk/pjsip.conf`. Чтобы сменить extension/пароль/адрес FreePBX после установки:

```bash
nano /opt/autodialer/.env
# поменять FREEPBX_IP / FREEPBX_EXTENSION / EXTENSION_PASSWORD

cd /opt/AutoDialer-Ultimate
sudo bash scripts/04_pjsip_config.sh     # безопасно перезаписывает pjsip.conf целиком

asterisk -rx 'core reload'
asterisk -rx 'pjsip show registrations'  # убедиться, что новый extension зарегистрирован
```

Не забудьте создать сам extension с этим номером на стороне FreePBX (Server-1) — иначе регистрация не пройдёт.
