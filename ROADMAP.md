...
Отдельно замечен (но не исправлен — вне объёма этого раунда) четвёртый
эффект: backend запускается через `gunicorn -w 4`, и все 4 воркера
независимо логинятся в AMI под одним и тем же аккаунтом `autodialer` —
Asterisk рассылает каждое событие всем подключённым менеджерам, поэтому
каждое реальное AMI-событие сейчас обрабатывается (и сохраняется в
`call_results`) 4 раза, по одному разу на воркер — `processed_events`
дедуплицирует только в рамках одного процесса. Остальные фоновые задачи
в `DialerManager` уже используют лидер-элекцию (см. лог запуска:
`лидер: True/False`), но она не применена к самой регистрации
AMI-обработчика. Стоит закрыть отдельно — не входит в текущий раунд.

Регрессионные тесты: `tests/test_dialer.py` (диспетчеризация
`handle_ami_event` по `event.event`, не `.name`; дедупликация по
`event`+`uniqueid`) и новый `tests/test_dialplan.py` (статическая
