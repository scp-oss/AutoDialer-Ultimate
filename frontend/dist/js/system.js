...
App.system = {
    ...
    async toggle() {
        ...
        if (this.state.enabled) {
            // Раньше текст обещал "все активные звонки будут сброшены" -
            // не соответствовало реальному поведению: system.disable_system()
            // (app/services/system.py) кладёт трубку на уже идущие звонки
            // только при force=True, а эта кнопка force вообще не передаёт
            // (POST /system/disable без query-параметров). Реально
            // происходит мягкая остановка - новые звонки не стартуют,
            // очередь дозвона очищается, но уже идущие разговоры
            // договаривают сами. Текст исправлен под фактическое (и
            // осознанно выбранное) поведение вместо обратного.
            if (!App.confirm('Аварийная остановка системы? Новые звонки не будут начинаться, очередь дозвона очистится. Уже идущие звонки договорят сами и завершатся штатно.')) {
                return;
            }
        }
